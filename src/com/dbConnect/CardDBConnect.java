...
public class CardDBConnect implements ICard {
	...
	public static boolean insertCardDetails(String id, String name, String nic, String address, 
		...
		try {
			...
			String sql = "insert into card(cardId,nic,name,address,email,phone,occupation,salary,cType,des) values('"+conId+"','"+nic+"','"+name+"','"+address+"','"+email+"','"+phone+"',"
	...
	public static boolean updateCardDetails(String accNo, String name, String nic, String address, 
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "UPDATE card SET cardId='"+convAccNo+"', nic='"+nic+"', name='"+name+"', address='"+address+"',"
					+ "email='"+email+"', phone='"+phone+"', occupation='"+occu+"', salary='"+convSal+"',cType='"+cardType+"'"
							+ "where cardId='"+convAccNo+"'";
			int rs = stmt.executeUpdate(sql);
	...
	public static boolean deleteCardDetails(String accNo) {
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "DELETE FROM card WHERE cardId='"+convAccNo+"'";
			int rs = stmt.executeUpdate(sql);
