...
public class CustomerDBConncet implements ICustomer{
    ...
    public static int updatecustomer(String id, String nic, String name, String email, 
    	...
    	int value = 0;
       	...
       	try {
           	con = DBConnect.getConnection();
           	stmt = con.createStatement();
        	String sql = "UPDATE customer SET id='"+id+"',nic='"+nic+"',name='"+name+"',email='"+email+"',address='"+address+"',phone='"+phone+"',"
        			+ "dob='"+dob+"',gender='"+gender+"',password='"+password+"' where id='"+id+"'";	    
        	int rs = stmt.executeUpdate(sql);
	...
	public static void createAcc(int AccNo,String nic,double amount) {
		
		int accno = AccNo;
		String Nic = nic;
		double amountVal=3500.;
		
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "insert into account values('"+accno+"','"+Nic+"','"+amountVal+"')";
			int rs = stmt.executeUpdate(sql);
	   	...
	   	try {
			...
			if(rs.next()) {
				...
				if(amount <= dbAmount) {
					...
					String transferSql = "insert into transaction(tid,recId,amount) values ('"+sendId+"','"+recId+"','"+amount+"')";
	...
	public static boolean sendMessage(String accNumber, String name, String email, String message) {
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "INSERT INTO message(accId,name,email,mess,time) VALUES('"+acNumber+"','"+name+"','"+email+"','"+message+"',CURRENT_TIMESTAMP())";
			int rs = stmt.executeUpdate(sql);
	...
	public static boolean makePayment(String accNo, String firstName, String lastName, String email, String cardNumber, 
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "insert into payment(pid,accNO,email,amount,cardNo,scode,pcode,fname,lname) values(0,'"+convAccNo+"','"+email+"','"+convAmount+"','"+cardNumber+"',"
	...
	public static boolean insertLoanDetails(String id, String name, String nic, String email, String phone,
		...
		try {
			con = DBConnect.getConnection();
			stmt = con.createStatement();
			String sql = "insert into loan (loanId,accNo,nic,email,phone,salary,loanAmount,description) values ('"+convId+"','"+name+"','"+nic+"','"+email+"','"+phone+"','"+convSalary+"','"+convLoan+"','"+description+"')";
			int rs = stmt.executeUpdate(sql);
